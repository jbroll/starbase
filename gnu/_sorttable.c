...
#ifdef SAOMOD_BINARY
	int BinarySort = 0;
#define	PutcNL(fp)	if ( BinarySort ) putc('\n', fp);
#else
#define	PutcNL(fp)	putc('\n', fp);
#endif
...
static int zip_factor 	  = 0;
static double mem_multiplier = 20;
static int sortalloc      = 1 * 1024 * 1024;
     ...
     char *file, *how;
{
  ...
  Malloc(xFile(fp)->data, 1024 * 1024);
  SetBuffer(fp, _IOFBF, xFile(fp)->data, 1024 * 1024);
#endif
#endif
...
tempname ()
{
  ...
  char *name = xmalloc (len + 32);
  struct tempnode *node =
  (struct tempnode *) xmalloc (sizeof (struct tempnode));

#ifdef SAOMOD_TABLE
  if ( zip_factor ) xFileZip(zip_factor); 
	...
	lenb = 0;
	  ...
	  if ( *texta != '+' && *texta != '-' && !ISDIGIT((int)*texta) ) {
	   ...
	   if ( key->nzone ) {
	     if ( Abs(va) < key->dzone ) {
		 if ( va < 0 ) {
		     va = -.1;	/* A constant value smaller than
				   key->dzone which can still carry a sign
				 */
		 }
		 if ( va > 0 ) {
		     va =  .1;
	     ...
	     if ( Abs(vb) < key->dzone ) {
		 if ( vb < 0 ) {
		     vb = -.1;	/* A constant value smaller than
				   key->dzone which can still carry a sign
				 */
		 } 
		 if ( vb > 0 ) {
     ...
     FILE *fp;
{
  ...
  initlines (&lines, mergealloc / linelength + 1,
	     ((int)(LINEALLOC * mem_multiplier)) / ((NMERGE + NMERGE) * sizeof (struct line)));
  alloc = linelength;
...
static void mergefps(fps, nfps, ofp)
     FILE *fps[], *ofp;
     register int nfps;
{
     ...
     FILE *ofp;
{
  ...
  initbuf (&buf, ((int)(sortalloc * mem_multiplier)));
  initlines (&lines, ((int)(sortalloc * mem_multiplier)) / linelength + 1,
	     ((int)(LINEALLOC * mem_multiplier)) / sizeof (struct line));
	  ...
	  if (digits[UCHAR (*s)]) {
		...
		switch (*s) {
		  ...
		  case 's':
		    ...
		    break;
		  case 'Z':
			zip_factor = atoi(argv[++i]);
			break;
		  case 'X':
			mem_multiplier = SAOstrtod(argv[++i], NULL);
			break;
		  case 't':
		    if (s[1]) tab = *++s;
		    else if (i < argc - 1) {
		  ...
		  case 'T':
		    ...
		    goto outer;
		    break;
		  case 'u': unique = 1; differ = 0; break;	/* First uniq only	*/
...
if ( !BinarySort ) {
  ...
  if ( checkonly ) exit (check (files, nfiles) != 0);
  ...
  if ( have_read_stdin && fclose (stdin)  == EOF ) perror (outfile);
  if ( ferror (stdout) || fclose (stdout) == EOF ) perror (outfile);
     ...
     int status;
{
  if (status != 0)
  ...
  else
    {
      ...
      fprintf(stderr,"or\n");
      fprintf(stderr,"+<type1>offset1.len1 ... (char data only)\n");
      fprintf(stderr,"\n");
      ...
      fprintf(stderr,"or more of Mbdfinr, this effectively disable global -Mbdfinr settings\n");
