  ...
  if (line->nfields >= line->nfields_allocated)
    {
      ...
      ReAlloc(line->fields, (line->nfields_allocated
					 * sizeof (struct field)));
    }
     ...
     struct seq *seq;
{
  seq->count = 0;
  seq->alloc = 1;
  Malloc(seq->lines, seq->alloc * sizeof (struct line));
}
     ...
     struct seq *seq;
{
  if (seq->count == seq->alloc)
    {
      seq->alloc *= 2;
	ReAlloc(seq->lines, seq->alloc * sizeof (struct line));
    }
     ...
     struct line *line2;
{
  if (outlist)
	...
	{
	  if ( !Force || i ) putchar (tab ? tab : ' ');
	  prfield (i, line1);
	}
      for (i = join_field_1 + 1 - Force; i < line1->nfields; ++i)
	{
	  if ( !Force || i ) putchar (tab ? tab : ' ');
	  prfield (i, line1);
	}
      ...
      for (i = 0; i < join_field_2 && i < line2->nfields; ++i)
	{
      ...
      for (i = join_field_2 + 1 - Force; i < line2->nfields; ++i)
	{
	  putchar (tab ? tab : ' ');
	  prfield (i, line2);
	}
     ...
     int field;
{
  ...
  if (file < 1 || file > 2 || field < 1)
    return 0;
  Malloc(o, sizeof (struct outlist));
  o->file = file;
  o->field = field;
  o->next = NULL;
     ...
     int count;
{
  int i;
  blank->nfields = count;
  Malloc(blank->beg, blank->nfields + 1);
  Malloc(blank->fields, sizeof (struct field) * count);
...
for ( i = 1; i < argc; i++ ) {
  ...
  if ( !(join_field_2 = table_colnum(T2, join_column2)) ) {
	...
	if ( !Force )
        ...
        { int lastcol = T1->header->ncol == join_field_1 ? join_field_1 - 1 : T1->header->ncol;

	    for ( i = 1; i <= lastcol; i++ )
		/* look up column name in the other table.
		 */
	      if ( Force || i != join_field_1 ) {
		    int c2 = table_colnum(T2, trim(table_colval(T1->header, i), buff));

		table_hdrfix(ofile, table_colval(T1->header, i)
		     , Force || (c2 != 0 && c2 != join_field_2) ? 1 : 0
		     , i == lastcol && T2->header->ncol == 1 ? '\n' : '\t');
	      }
	}
        ...
        { int lastcol = T2->header->ncol == join_field_2 ? join_field_2 - 1 : T2->header->ncol;
            ...
            for ( i = 1; i <= lastcol; i++ )
              if ( Force || i != join_field_2 ) {
		    int c1 = table_colnum(T1, trim(table_colval(T2->header, i), buff));

                table_hdrfix(ofile, table_colval(T2->header, i)
                     , Force || (c1 != 0 && c1 != join_field_1) ? 2 : 0
                     , i == lastcol ? '\n' : '\t');
	      }
        }
	...
	if ( !Force ) 
	    ...
	    for ( i = 1; i <= lastcol; i++ )
		if ( Force || i != join_field_1 ) FPrint(ofile, "%s%c"
				    , T1->dashes->column[i]
				    , i == lastcol && T2->header->ncol == 1 ? '\n' : '\t');
	}
        ...
        { int lastcol = T2->header->ncol == join_field_2 ? join_field_2 - 1 : T2->header->ncol;
            ...
            for ( i = 1; i <= lastcol; i++ )
                if ( Force || i != join_field_2 ) FPrint(ofile, "%s%c"
                                    , T2->dashes->column[i]
                                    , i == lastcol ? '\n' : '\t');
        }
