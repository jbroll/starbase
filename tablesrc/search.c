	    ...
	    void	 *filecache;
{
	...
	*here = '\0';
    ...
    for ( i = 0; i < naxes; i++ ) {
    for ( j = 0; j < naxes; j++ ) {
		char	 buf1[TABLE_TRIMBUF];
		char	 buf2[TABLE_TRIMBUF];
	...
	col1 = trim(table_colnam(search->THead, axes[i]->c2), buf1);
	col2 = trim(table_colnam(search->THead, axes[j]->c2), buf2);

	sprintf(pattern, "%s.%s.%s", here, col1, col2);

	if ( (name = table_searchcheckindexpattern(search, directo, pattern, 0)) ) {
	...
	char	*option;
{
	...
	csys->axis[i].type   = '2';
        ...
        csys->axis[i].once   =  0;
        csys->axis[i].R      =  NULL;
        csys->axis[i].method = TSEARCH_NUMBER;
    }
    return "C";
}
	...
	TableRow	 row;
{
    ...
    if ( csys->rcols[0] > 0 ) {
	csys->range[0] = table_colvald(row, csys->rcols[0]);
    }
	...
	TableRow	 row;
{
    ...
    if ( csys->rcols[1] > 0 ) {
	csys->range[1] = table_colvald(row, csys->rcols[1]);
    }
    ...
    switch ( csys->data[2] ) {
     case 'h': width = h2d(csys->range[1]);	break;
     case 'd': width =     csys->range[1];	break;
     case 'r': width = r2d(csys->range[1]);	break;
    }
...
int annufilte(csys, row1, row2)
	CsysDef		*csys;
	TableRow	row1;
	TableRow	row2;
{
		double	r1, r2, d1, d2;
		double	radius0, radius1, sep;
	...
	switch ( csys->data[2] ) {
         case 'h': radius0 = h2r(csys->range[0]);	break;
         case 'd': radius0 = d2r(csys->range[0]);	break;
         case 'r': radius0 =     csys->range[0];		break;
        }
	switch ( csys->data[2] ) {
         case 'h': radius1 = h2r(csys->range[1]);	break;
         case 'd': radius1 = d2r(csys->range[1]);	break;
         case 'r': radius1 =     csys->range[1];	break;
        }

	sep = slaDsep(r1, d1, r2, d2);

	if ( sep >= radius0 && sep <= radius1 )	return 1;
	else					return 0;
}
        ...
        char            *option;
{
	...
	csys->axis[0].type   = '2';
        csys->axis[0].once   =  0;
        csys->axis[0].R      =  NULL;
	csys->axis[0].method = TSEARCH_NUMBER;
	return "211";
}
        ...
        TableRow         row;
{
	...
	if ( csys->rcols[0] == 0 && csys->rcols[1] == 0 ) {
	    csys->axis[0].once = 1;
            csys->axis[0].R    = NULL;
	}
...
CsysType CsysTypes[] = {
	...
	, { 'A',	"annular",	annufilte, annusetup, annuparse }
	...
	File		 file;
{
	...
	if ( d->posit ) {
	    fprintf(d->ofile, "%lld	%lld\n", (long long) start, (long long) (end-start));
	    return data;
	}
	...
	if ( d->cache ) {
	    ...
	    if ( cache_hashe((Cache) d->cache, &node, NULL) != NULL ) {
		return data;
	    }

	    cache_cache((Cache) d->cache, &node);
	}
	if ( d->setcnt ) { fprintf(d->ofile, "%d\t", d->setcnt); }

	if ( d->T1 ) {
	    table_rowput(d->ofile, d->T1, d->R1, NULL, NULL, 0);

	    for ( i = d->T1->header->ncol; i < d->R1->ncol; i++ ) { printf("\t"); }
	}
  ...
  AFL_FUZZ_FILE("/tmp/table.tab");
	...
	for ( n = 1; n < argc; n++ ) {
		...
		if ( F2 ) {
		   ...
		   arg = NULL;
		    ...
		    if ( (End = Seek(F2, 0    , SEEK_END)) == (off_t) -1 ) {
			perror("search:");
			exit(1);
		    }
		...
		Malloc(csys[nsys].rcols, sizeof(int)    * csys[nsys].nrange);
	    }
		...
		if ( *args == 'C' || *args == '1' ) {
		  if ( T1 == NULL && (T1 = table_header(F1, 0)) == NULL )
	    	    except(ex_search
		...
		if ( *here ) {
		    ...
		    if ( !(csys[nsys].rcols[i] = table_colnum(T1, argv[n])) ) {

			except(ex_search, "this doesn't look like a numeric range or a column from the key table: %s"
				    , argv[n]);
		    }
		}
	    }
	...
	if ( T1 && joiner ) {
		...
		for ( j = 1; j <= T1->header->ncol; j++ )
		     ...
		     */
		    lcol[ncol++] = table_hdrfix(ofile, table_colval(T1->header, j)
				,  table_colnum(T2
				, trim(table_colval(T1->header, j), buf)) ? 1 : 0
				       ,  '\t');

		for ( j = 1; j <= T2->header->ncol; j++ )
		    lcol[ncol++] = table_hdrfix(ofile, table_colval(T2->header, j)
				,  table_colnum(T1
				, trim(table_colval(T2->header, j), buf)) ? 2 : 0
				       ,  j == T2->header->ncol ? '\n' : '\t');
	...
	jdata.setcnt= setcnt;
        ...
        if ( setcnt ) jdata.print = 1;

    	if ( T1 ) table_rowbuf_open(F1, T1);

	if ( cross ) { start = Seek(F1, 0, 1); }

	if ( setcnt == 1 ) {
	    jdata.cache = (RCache) cache_init(sizeof(struct RCache), 17967, 0
				, NULL, NULL, rowcmp);
	} else {
	    jdata.cache = NULL;
	    if ( setcnt == 2 ) { jdata.setcnt      = 1; }
	}
	...
	while ( T1 == NULL 
	    ...
	    if ( cross && jdata.cache ) {
		struct	_CNode		   node;

		node.hash = start;
	        start += row->nbuf;

		if ( cache_hashe((Cache) jdata.cache, &node, NULL) != NULL )  {
	   	   jdata.setcnt++;
		   continue;
		}
	    }
	    ...
	    nrow++;
	    if ( T1 && T1->header->ncol != row1->ncol ) {
		fprintf(stderr, "search: row %d contains %d columns, there are %d in the input header; the output table maybe corrupted.\n"
			, nrow, row1->ncol, T1->header->ncol);
		if ( T1->header->ncol < row1->ncol ) {
		    row1->ncol = T1->header->ncol;
		}
	    }
		...
		if ( axes[i]->once ) {
		    if ( !axes[i]->R ) {
			axes[i]->R    = range_copy(range_union(r));
			axes[i]->type = 'R';
		    }
		} else r = range_union(r);
		...
		if ( joiner == 2 ) {					/* Left Join	*/
		    if ( setcnt ) { fprintf(ofile, "%d\t", jdata.setcnt++); }

		    table_rowput(ofile, T1, row1, NULL, NULL, 0);

		    for ( i = 1; i < T2->header->ncol; i++ ) { printf("\t"); }
		    printf("\n");
		}
	    }
   ...
   if ( debug ) {
	for ( i = 0; i < naxes; i++ ) {
	    fprintf(stderr, "Axis % 2d Laddr:	% 10d/% 10d	Access:	% 10d/% 10d\n", i+1
		, axes[i]->search->ltry, axes[i]->search->lhit
		, axes[i]->search->cache->try, axes[i]->search->cache->hit);
	}
   }
#endif
   exit(0);
