...
split_field0()
{
   ...
   else
   {
	 ...
	 case C_SPACE:
	    nf = space_split(string(cp)->str, string(cp)->len, NCols) ;
	    break ;
   ...
   CELL *cp ;
{
	...
	*/
	 ...
	 {
		int	type, width, prec, flag;

	     SAOparsefmt(string(fp)->str, &type, &width, &prec, &flag);
	     fp->vcnt = prec-3 < 0 ? 0 : prec-3;
	 }	
	    cellcpy(OFMT, cp) ;
	    OFMT->vcnt = fp->vcnt;
	 }
   ...
   else
   {
	 ...
	 {			/* use the string field temporarily */
	    ...
	    else  /* its a double */
	    {
	       ...
	       if ( cp->vcnt ) {
	 		 SAOconvert(xbuff, cp->dval, cp->vcnt & ~0x80
...
slow_cell_assign(target, source)
   ...
   CELL *source ;
{
   if ( target >= field && target <= LAST_PFIELD ) {
