...
scan_open()			/* open pfile_name */
{
   ...
   {
      perrmsg(errno, "cannot open %s", pfile_name) ;
      mawk_exit(2) ;
   }
}
...
/* Move these from down below so that I can get at them
 */
   ...
   *flag = 0 ;
   ...
   else
   {
	int	unsi = 0;
      ...
      *p++ = next();
      if ( p[-1] == 'x' || p[-1] == 'o' || p[-1] == 'b' ) {
	SAOdtype = p[-1];


	switch ( SAOdtype ) {
	 case 'x': while ( (*p++ = next()) && ishex(p[-1]) ) ;		break;
	 case 'o': while ( (*p++ = next()) && isoctal(p[-1]) ) ;	break;
	 case 'b': while ( (*p++ = next()) && isbin(p[-1]) ) ;		break;
	}

	if ( scan_code[p[-1]] == SC_DIGIT ) {
		*flag = UNEXPECTED ;
		 yylval.ival = p[-1];
		 return 0.0 ;
	}
	...
	if ( unsi ) {
	    ...
	    d = strtoul(&string_buff[2], (char **) &endp, cvbase(SAOdtype));
	    SAOdtype |= 0x80;
	} else {
	    d = strtol(&string_buff[2], (char **) &endp, cvbase(SAOdtype));
	}
   ...
   d = SAOstrtod(string_buff, (char **) &endp) ;
