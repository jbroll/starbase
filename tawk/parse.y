...
%type <start>  block  block_or_separator separator
...
%type <start>  expr cat_expr p_expr f_expr
...
%type <ival>   arglist args assignlist exprlist
...
program_block :  PA_block   /* pattern-action */
              |  function_def
              |  outside_error block
	      | LOAD STRING_ STRING_ NL {
		load(((STRING*) $2)->str, ((STRING*)$3)->str);
		}
              ;
...
p_expr  :   DOUBLE
          {  $$ = code_offset ;
                 code2(_PUSHD,     $1); code1(SAOdtype); break;
          }
      |   STRING_
...
return_statement :  RETURN   separator
	    ...
	    fprintf(stderr, "tawk: return statement not in function block\n");
		      ...
		      active_funct->nrets = 0;
                      code1(_RET0) ; }
              |  RETURN   LBOX exprlist RBOX separator
                    { 
			if ( active_funct == NULL ) {
	    fprintf(stderr, "tawk: return statement not in function block\n");
			    exit(1);
			}
                        active_funct->nrets = $3;
                        code1(_RET2);
                        $$ = code_offset; 
                    }
	    ...
	    fprintf(stderr, "tawk: return statement not in function block\n");
			...
			$$ = $2 ; active_funct->nrets = 1; code1(_RET) ; }
	;
...
statement  :  LBOX mark assignlist RBOX ASSIGN f_expr
        { $$ = $2;
	  code1(_ASSIGN2);
	  xcode2(-$3, FReturns);
	}
...
statement  :  LBOX mark assignlist RBOX ASSIGN LBOX exprlist RBOX separator
        {
	  $$ = $2;
          code1(_ASSIGNL) ;
          code1(-$3);
...
assignlist : fvalue COMMA fvalue
        {       $$ = 2;         }
           | assignlist COMMA fvalue
        {       $$++;           }
	;
...
f_expr  :  FUNCT_ID mark  call_args
           { $$ = $2 ;
             code2(_CALL, $1) ;
