...
/* HALT only used by the disassemble now ; this remains
	...
	*/
	 ...
	 case _POP:
	    ...
	    for ( i = 1; i <= nrets; i++ ) {
		if ( i <= nassign ) {
            	    if ( (sp-(nrets-i))->type == C_MBSTRN )
			check_strnum(sp-(nrets-i)) ;
		    slow_cell_assign((sp-(nassign+nrets-i))->ptr, sp-(nrets-i)) ;
		}
                cell_destroy(sp-(nrets-i)) ;
	    }
	 ...
	 case _POW_ASG:
	    ...
	    /* will anyone ever use these ?  Yes, they will Mike !!*/
	 ...
	 case _RET:
      ...
      case _RET2   : {
			...
			int i;
              ...
              for ( i = nrets; i <= nrets; i++ ) {
		/* move the return values */
              	(void) cellcpy(old_sp+1, sp + i - 1) ;
              	cell_destroy(sp + i - 1) ;
	      }
              zfree(stack_base, sizeof(CELL)*EVAL_STACK_SIZE) ;
              stack_base = old_stack_base ;
	 ...
	 case _CALL:
	    ...
	    */

	    {
	       ...
	       int   nrets = fbp->nrets == 0 ? 1 : fbp->nrets;
		  ...
		  if (*type_p == ST_LOCAL_ARRAY)
	       ...
	       if ( fbp->code == (void *) 1 )
                (*fbp->addr)(sp, fbp->nargs, a_args);
               else
	        execute(fbp->code, sp, nfp, nrets) ;
		  ...
		  cp = sp + nrets ;	 /* cp -> the function return */
		  ...
		  /* Copy return values up the stack.
                   */
                  for ( i = nrets; i--; ) {
                          (void) cellcpy(++sp, cp - i) ;
                          cell_destroy(cp - i) ;
                  }
	       } else  sp += nrets ;	    /* no arguments passed */
	    }
	    break ;
