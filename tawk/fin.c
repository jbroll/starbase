   ...
   int fd, main_flag ;
{
       ...
       || interactive_flag )
   {
      ...
      {
	 perrmsg(errno, "fdopen failed") ; mawk_exit(2) ; 
      }
      if ( interactive_flag ) setbuf(fin->fp, (char *)0);
   }
   else	 fin->fp = (FILE *) 0 ;
   ...
   int main_flag ;
{
   ...
   if (filename[0] == '-' && filename[1] == 0)
   {
      ...
      else 	setmode(0, O_TEXT);
#endif
      return FINdopen(0, main_flag) ;
   }

   if ((f = xOpen(filename, "r")) == NULL) {
      perrmsg(errno, "cannot open %s", filename) ;
      return (FIN *) 0 ;
   }

   return FINdopen(fileno(f), main_flag) ;
}
   ...
   int open_flag ;		 /* called by open_main() if on */
{
   ...
   while (argi < argc.dval)
   {
	 ...
	 continue ;		 /* its deleted */
      ...
      {
         perrmsg(errno, "cannot open %s", string(cp)->str) ;
	 mawk_exit(2) ;
      }
