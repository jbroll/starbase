...
out_failure:
   perrmsg(errno, "cannot open \"%s\" for output", name) ;
   mawk_exit(2) ;
   ...
   FILE *fp ;
{
   if (fflush(fp) < 0)
   {
      perrmsg(errno, "unexpected write error") ;
      mawk_exit(2) ;
   }
}
   ...
   int *pid_ptr ;
{
   ...
   switch (*pid_ptr = fork())
   {
	 ...
	 perrmsg(errno, "failed to exec %s -c %s", shell, name) ;
	 fflush(stderr) ;
	 _exit(128) ;
   ...
   char *name, *mode ;
{
   ...
   FILE *retval = xOpen(name, mode) ;

      tbuffer = malloc(1024*1024);
      setbuffer(retval, tbuffer, 1024*1024);
...
stdout_init()
{
	...
	if (binmode() & 2) {
      	    ...
      	    setmode(1,O_TEXT) ; setmode(2,O_TEXT) ; 
        }
    } else {
      	    setmode(1,O_TEXT) ; setmode(2,O_TEXT) ; 
    }
