...
bad_option(s)
   char *s ;
{
   perrmsg(0, "not an option: %s", s) ; mawk_exit(2) ; 
}
   ...
   char **argv ;
{
   ...
   for (i = 1; i < argc && argv[i][0] == '-'; i = nextarg)
   {
	 ...
	 if (i == argc - 1 && argv[i][1] != '-')
	 {
	    ...
	    {
	       perrmsg(0, "option %s lacks argument", argv[i]) ;
	       mawk_exit(2) ;
	    }
	    bad_option(argv[i]) ;
	 }
	 ...
	 case 'W':
	    ...
	    else if (optarg[0] == 'E')
	    {
	       if ( pfile_name )
	       {
		  perrmsg(0, "-W exec is incompatible with -f") ;
		  mawk_exit(2) ;
	       }
	    ...
	    else  perrmsg(0, "vacuous option: -W %s", optarg) ;
	 ...
	 case 'v':
	    ...
	    {
	       perrmsg(0, "improper assignment: -v %s", optarg) ;
	       mawk_exit(2) ;
	    }
	    break ;
