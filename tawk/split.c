...
static int PROTO(space_ov_split, (char *, char *, int, int)) ;
...
#define EAT_NON_SPACE()	  					\
  ...
  if ( i+1 == ncol ) {						\
   ...
   /* we've overflowed */
   return i + space_ov_split(s, back, i, ncol) ;
   ...
   register CELL *sp ;
{
   ...
   if (string(sp)->len == 0)	/* nothing to split */
	 ...
	 case C_SPACE:
	    cnt = space_split(string(sp)->str, string(sp)->len, -1) ;
	    break ;
